...
import { getAllLeaveRequestsAction } from "../redux/EmployeeLeaveCheckReducer"; 
...
export const EmployeeLeaveCheck = () => {
  const formEl = useRef();
  ...
  const CheckLeaveList = (e) => {
    ...
    if (isFormValid) {
      ...
      console.log(taskId);
      dispatch(getAllLeaveRequestsAction( taskId ));
      
      history.push("/employee-leave-checklist");
      ...
      >
        <div className="w-50">
          <h4 className="text-center text-light alert alert-transaparent">
            Employee Leave Request Status Check
          </h4>
