...
import { getAllLeaveRequestAction } from "../redux/AdminLeaveApprovalListReducer";
import { updateEmployeeLeaveRequestAction, updateRenderAction } from "../redux/EmployeeLeaveRequestReducer";
...
export const AdminLeaveApprovalList = () => {
  ...
  // 2
  const updateLeaveRequest = (item) => {
    console.log("Update Record", item);

    // 3 :: updating the store
    dispatch(updateRenderAction(item));

    // dispatch(updateEmployeeLeaveRequestAction(item));

    // navigateing to the page
    history.push("/admin-leave-update");
  };

  console.log(state.adminLeaveApprovalRequest.AdminLeaveApprovalList);
  return (
    <div>
       <div>
      ...
      <table className="table">
        ...
        <tbody className="text-light">
          {state. adminLeaveApprovalRequest.AdminLeaveApprovalList.map(
            (item, index) => (
              <tr key={index}>
                ...
                <td>
                  <input
                    ...
                    onClick={() => updateLeaveRequest(item)}
                  />
                </td>
              </tr>
            )
